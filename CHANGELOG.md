...
### Added
- **Scoop install module** (`src/modules/package_manager/scoop_install.py`, `platforms={"win32"}`). Windows analogue to Homebrew for **non-administrator** setups — Scoop installs everything under `~/scoop/` (user-writable) and **never asks for UAC**. Installer route: `powershell -Command "Set-ExecutionPolicy RemoteSigned -Scope CurrentUser -Force; irm https://get.scoop.sh | iex"`. Idempotency via `shutil.which("scoop")` + the canonical `~/scoop/shims/scoop.cmd` / `.ps1` paths. Tells the user to open a new Git Bash window so `scoop` lands on PATH for subsequent modules.
- **Scoop packages module** (`src/modules/package_manager/scoop_packages.py`, `platforms={"win32"}`). Mirrors `homebrew_packages` — picker over a curated `ScoopPackage(name, description, bucket)` list with Install/Uninstall/Back action menu, already-installed items filtered out via `scoop list`. Auto-adds the required Scoop buckets (`extras`, `nerd-fonts`) before installing packages that need them. Selected packages are installed in a single `scoop install <pkg1> <pkg2> ...` batch (or `scoop uninstall`). **No admin, no UAC.** Initial list: `7zip`, `gh`, `mise`, `brave`, `FiraCode-NF`, `windows-terminal`, `vscode`, `idea-community`, `bruno`, `mockoon`, `beekeeper-studio`, `bitwarden`, `oh-my-posh`.

### Changed
- **Mise runtimes "not found" hint is now platform-aware.** On Windows the message points to `Scoop packages (pick mise)` instead of `Homebrew packages`.

### Removed
- **Chocolatey modules dropped before release.** The Unreleased section previously added `chocolatey_install` and `chocolatey_packages` modules. Chocolatey is fundamentally admin-only — its package store lives under `C:\ProgramData\chocolatey` and most casks need to write into `Program Files`. Targeting a non-administrator user (the intended setup for this project on Windows) made every Choco install fire a UAC prompt or fail outright. Replaced by Scoop, which is user-level by design. Both `chocolatey_install.py` and `chocolatey_packages.py` are deleted; no shipped release ever contained them.
...
- **Claude Code `cc` alias and the `claude` binary both invisible in new Git Bash windows.** Two compounded login-shell quirks on Windows: (1) Git Bash opens as a **login shell**, so it reads `~/.bash_profile` (or `~/.profile`) and only loads `~/.bashrc` when that file does an explicit `source ~/.bashrc`; the `cc` alias never made it across. (2) The Claude Code (and uv) installers drop a `~/.local/bin/env` script that prepends `~/.local/bin` to `PATH`, but Git Bash does not source it automatically — so `claude.exe` and `uv.exe` exist on disk yet `claude: command not found`. `claude_code.py` now wires **both** lines into `~/.bash_profile` (idempotent — checks each line separately, only appends what's missing):
  ...
  The bridge step runs in **every** path of the module (already-installed, dry-run, fresh install), so re-running the module on a Windows box where Claude Code is already present is enough to apply the fix.
