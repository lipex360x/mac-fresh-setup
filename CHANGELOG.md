...
- Module: **Homebrew formulae** (`src/modules/homebrew_formulae.py`). Curated list of CLI tools shown in a `questionary.checkbox` (defaults unchecked); each row shows the formula name with a description tooltip on hover. Initial list: `mise` (polyglot runtime manager, replaces asdf) and `gh` (GitHub CLI). Skips formulae already installed (`brew list --formula --versions`); runs `brew install <name>` with live output for each selected.
- Module: **Homebrew casks** (`src/modules/homebrew_casks.py`). Same pattern as formulae but for GUI apps. Initial list: `iterm2`. Runs `brew install --cask <name>` with live output.
- Both checkbox menus now include an explicit `← Back` choice; picking it (with nothing else, or alone) returns to the module menu without installing anything.
- Already-installed formulae/casks are shown as `disabled` in the checkbox (greyed out, unselectable, with reason "installed"). A single `brew list --formula --versions` / `brew list --cask --versions` is queried once before the prompt.
- New category **Styling** with module **iTerm2 preferences** (`src/modules/iterm2_prefs.py`). Reads the plist source from the `ITERM2_PREFS_URL` env var (or prompts if unset), fetches bytes from a URL or local path, backs up any existing `~/Library/Preferences/com.googlecode.iterm2.plist`, writes the new file, and runs `killall cfprefsd` to invalidate macOS's preference cache. On any write failure, falls back to `~/Downloads/com.googlecode.iterm2.plist` and prints manual import instructions (cp + killall cfprefsd, or iTerm2's "Load settings from custom folder or URL").
